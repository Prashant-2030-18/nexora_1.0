...
# Safe ALTER for columns added after initial SQLite create
try:
    ...
    def _ensure_cols():
        ...
        if "sachet_etag_cache" in tables:
            ...
            extra = [
                ...
                ("fetched_at", "ALTER TABLE sachet_etag_cache ADD COLUMN fetched_at DATETIME"),
            ]
        ...
        if "user_reports" in tables:
            ...
            ur_extra = [
                ...
                ("image_analysis_available", "ALTER TABLE user_reports ADD COLUMN image_analysis_available BOOLEAN DEFAULT 0"),
                ("image_analysis_summary", "ALTER TABLE user_reports ADD COLUMN image_analysis_summary TEXT"),
                ("verified_at", "ALTER TABLE user_reports ADD COLUMN verified_at DATETIME"),
                ...
                ("expires_at", "ALTER TABLE user_reports ADD COLUMN expires_at DATETIME"),
            ]
        ...
        if "users" in tables:
            ...
            if "sms_alerts_enabled" not in existing_u:
                alters.append("ALTER TABLE users ADD COLUMN sms_alerts_enabled BOOLEAN DEFAULT 0")
        if alters:
            with _eng.begin() as conn:
                for sql in alters:
                    conn.execute(_text(sql))
            print(f"[SCHEMA] Applied {len(alters)} SQLite column migration(s).")
    _ensure_cols()
except Exception as _mig_err:
    print(f"[SCHEMA] Migration notice: {_mig_err}")
...
def health():
    ...
    return {
        ...
        "database": "postgresql" if not (settings.ENVIRONMENT == "development") else "sqlite",
    }
...
def services_status():
    ...
    try:
        ...
        return {
            ...
            "database": {"status": "Connected" if db_ok else "Unavailable", "detail": "SQLite"},
            "sachetNdma": {
                "status": sachet.get("statusBadge") or sachet.get("status"),
                "statusCode": sachet.get("statusCode"),
                "isLive": sachet.get("is_live"),
                "lastSuccessfulFetch": sachet.get("lastSuccessfulFetch") or sachet.get("last_successful_fetch_time"),
                "etagPresent": sachet.get("etagPresent"),
